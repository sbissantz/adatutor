...
vignette: >
  %\VignetteIndexEntry{A Hands-On Tutorial on Adaptive Boosting for
  Psychologists: Predicting Replication Success with AdaBoostTutorial}
...
cstump <- rpart(replicate ~ ., data = train[, featnms],
...
# Fit classification stump 
h1 <- rpart(replicate ~ ., data = train[, featnms],  method = "class", 
            weights = D1, control = treehypar)
``` 
...
# Unormalized weights (t2)
D2_raw <- D1 * exp(-alpha1 * chi1)
# Normalization constant
Z1 <- sum(D2_raw)
# Normalized weights (t1)
D2 <- D2_raw / Z1
```
...
```
# Module 4: Leave-Project-Out Cross-Validation
...
```{r listing_33}
# Fit optimized model
fit_final <- trainAda(replicate ~ ., data = df[, featnms], 
