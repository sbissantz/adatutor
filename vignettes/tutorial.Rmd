...
cstump_ast <- rpart(replicate ~ .,  data = train[,featnms],
                  method = "class", weight = D, control = treehypar)
# Visualize
...
D <- rep(1, m)/m
for (t in seq_len(T)) {
  h <- rpart(replicate ~ ., data = train[, featnms],
...
for(i in seq_len(hgrid_rows)) {
  ...
  runtime <- system.time({
    ...
    for (rp in c("ee", "ml1", "ml3", "rpp", "ssrp")) {
      ...
      train <- df[!(df$pid %in% rp), ]
      # Fit model to data 
      fit <- trainAda(replicate ~ ., data = train[, featnms], T = hgrid$T[i], eta = hgrid$eta[i], treehypar = rpart.control(maxdepth = hgrid$depth[i]), verbose = FALSE)
      # Make predictions on test set
      ypred <- testAda(fit, data = test[, prednms], verbose = FALSE)
      # Store performance evaluations 
      hgrid[[rp]][i] <- auc(roc(ypred, y_test))
      }
