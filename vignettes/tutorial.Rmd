...
yretro <- predict(h1, newdata = train[, featnms], type = "class")
# Retrodictions do not match observations
...
```{r listing_21}
# Retrodictions match observations
...
for (t in seq_len(T)) {
  ...
  yretro <- predict(object = h, newdata = train[, featnms], type = "class")
...
```{r listing_26}
for (n in seq_len(m)) {
  yretro_ada[n] <- sign(sum(a * yretro_stumps[n, ]))
}
```
...
plot(NULL, xlim = range(x1_seq), ylim = range(x2_seq),
     xlab = "power.o", ylab = "n.o")
points(grid, col = grid_colors, pch = ".")
# points(grid, col = as.integer(ypred) + 6L, pch = ".")
z_mat <- matrix(as.integer(ypred), nrow = resolution)
# For the black line
contour(x1_seq, x2_seq, z_mat, add = TRUE, drawlabels = FALSE, lwd = 2,
        col = "black", levels = (1:(k-1))+.5)
...
TODO: "object 'y_test' not found".

```{r listing_27}
# Important: Test set
y_test <- test[, outnme]
# Predictions (test data)
ypred_stumps <- sapply(h, predict, newdata = test[, featnms], type = "vector")
ypred_stumps <- 2 * ypred_stumps - 3
ypred_ada <- sign(a %*% t(ypred_stumps))
# Confusion matrix
table(y_test, ypred_ada)
# Inperfect predictions!
```
