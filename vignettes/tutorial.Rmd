...
```{r}
confusion <- table(ypred, test$replicate)
# Accuracy
sum(diag(confusion)) / sum(confusion) 
# Area under the curve
library(AUC)
roc(ypred, test$replicate) |> AUC::auc()
# Matthew correlation 
...
Do a leave-project-out cross-validation. Note that this runs for approximately 4
minutes.

```{r listing_30}
# Partition the data by pid 
...
runtime <- system.time({
  ...
  for (rp in c("ee", "ml1", "ml3", "rpp", "ssrp")) {
    ...
    fit <- trainAda(replicate ~ ., data = train[, featnms],
                    T = 1e4, eta = 1,  # Boosting hyperparameter
                    treehypar = treehypar) # Tree hyperparameter
    # Make predictions on test set
    ypred <- testAda(fit, data = test[, prednms])
    # Store the performance evaluation 
...
## Hyperparameter Optimization using LPO-CV
...
Do hyperparameter optimization using a leave-project-out cross-validation
strategy. Note that this runs for approximately 5 Minutes.

```{r listing_32}
hgrid_rows <- nrow(hgrid)
# Partition the data by pid 
df_split <- split(df[, voinms], df$pid)
# For each entry in the hypergrid...
for(i in seq_len(hgrid_rows)) {
  # Track the iteration time 
  runtime <- system.time({
    ...
    for (rp in c("ee", "ml1", "ml3", "rpp", "ssrp")) {
      ...
      fit <- trainAda(replicate ~ ., data = train[, featnms], T = hgrid$T[i],  
                      eta = hgrid$eta[i], treehypar = rpart.control( 
                      maxdepth = hgrid$maxdepth[i]), verbose = FALSE)
      # Make predictions on test set
...
```{r}
fit_final <- trainAda(formula = replicate ~ ., data = df[, featnms], 
                      T = 10, eta = 1, treehypar = rpart::rpart.control(
                        maxdepth = 1))
```
