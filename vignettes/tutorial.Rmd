...
vignette: >
  %\VignetteIndexEntry{A Hands-On Tutorial on Adaptive Boosting for Psychologists: Predicting Replication Success with AdaBoost}
...
```{r figure_3, fig.width=7, fig.asp = .75}
# Outcome variable
...
```{r figure_4, fig.width=5, fig.asp = 1}
...
```{r figure_6, fig.width=5, fig.asp = 1}
...
```{r figure_7, fig.width=5, fig.asp = 1}
...
```{r figure_8, fig.width=5, fig.asp = 1}
n_25 <- 25
scaling <- 175
...
```{r figure_9, cache=TRUE, fig.width=7, fig.asp = .75}
# Outcome vector
yobs <- train[, outnme]
...
fit <- trainAda(replicate ~ .,
                data = train[, voinms],
                T = 1e3, eta = 1,  # Boosting hyperparameter
                # T = 1e2, eta = 1,  # Boosting hyperparameter
