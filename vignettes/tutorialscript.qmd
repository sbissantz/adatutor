...
roc.curve(scores.class0 = ymargin, weights.class0 = as.integer(y_test) - 1)$auc
...
::: {.callout-important}
## `type = "class"` gives balanced accuracy, not AUC
Switched to `PRROC` package. Cite and fix in the tutorial.
:::
...
runtime <- system.time({
  ...
  for (rp in c("eerp", "ml1", "ml3", "rpp", "ssrp")) {
    ...
    auc[[rp]] <- roc.curve(
      scores.class0 = ypred,
      weights.class0 = as.integer(test$replicate) - 1
    )$auc
  }
})
